...
object GellyExample extends App {
  ...
  val max_out = mostUsedGraph.outDegrees().maxBy(1).collect().get(0).f1
  val max_in  = mostUsedGraph.inDegrees().maxBy(1).collect().get(0).f1

  val vertexMaxOutDeg = mostUsedGraph.outDegrees().filter(_.f1 == max_out).collect() // Najczęściej używane stacje jako startowa
  val vertexMaxInDeg = mostUsedGraph.inDegrees().filter(_.f1 == max_in).collect() // Najczęściej używane stacje jako końcowa


  println("Zad.9 Najczęściej używane stacje dla " + mostUsedBike + " (nazwa, licznik)")
  println("\ta. jako startowe: " + vertexMaxOutDeg)
  println("\tb. jako końcowe: " + vertexMaxInDeg)
}
